...
file (`v0_77` or `v1_06`), followed by the two version axes the file states
(`MetaData.Version`): the application version from `map/@version` and the schema
version from `map/@schema`. A classic file states no schema at all, and reports
...
info:	testdata/blank-2017-1.77-1.0.wxx
	   v0_77 codec: app 1.77, schema implicit (classic)
	       5 tiles high
	       3 tiles wide
	       1 terrain tiles defined
```
