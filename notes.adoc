...
There are two independent version axes (see `docs/adr/0003-version-axes.md`):
the *application* version (the Worldographer build that wrote the file) and the
*XML schema* version (the on-disk data format). They do not always line up, and
the schema version is not always written to disk.
...
3. map.schema identifies the XML Schema version. It is present only in *later*
   W2025 files; classic (H2017) and *early* W2025 files omit it, so their schema
   version is implicit (inferred from `release`/`version`, not read from an attribute).
...
|UTF-16/BE||1.77||1.0.0
|UTF-16/BE|2025|2.06|1.06|1.6.0
|===
