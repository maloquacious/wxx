...
*XML schema* version (the on-disk data format). They do not always line up: a
W2025 file carries `version="2.06"` alongside `schema="1.06"`.
...
3. map.schema identifies the XML Schema version. It is present in W2025 files;
   classic (H2017) files omit it, so their schema version is implicit (inferred
   from the absence of `release`/`schema` plus a `1.x` `version`).
